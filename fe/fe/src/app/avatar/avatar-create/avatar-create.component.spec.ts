...
describe('AvatarCreateComponent', () => {
  ...
  it('#should properly react on create avatar', () => {
    ...
    fixture.detectChanges();
    const action = new AvatarStoreActions.AvatarCrudActions.CreateRequest(avatar);
  ...
  it('#should properly react on create avatar fail and handle errors from formData', () => {
    ...
    dispatchSpy = spyOn(store, 'dispatch');
    component.createFailed(formData);
    fixture.detectChanges();
    expect(dispatchSpy).toHaveBeenCalledWith(action);
  });

  it('#should cancel avatar create by press "Cancel" button', () => {
    const cancelAvatarCreate = spyOn(component, 'cancelAvatarCreate');
    fixture.detectChanges();
